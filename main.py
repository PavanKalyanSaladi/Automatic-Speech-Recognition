...
async def process_file(audioFile: UploadFile = File(...)):
    file_location = f"{UPLOAD_DIRECTORY}/{audioFile.filename}"
    with open(file_location, "wb") as file_obj:
        shutil.copyfileobj(audioFile.file, file_obj)

    # Convert the uploaded file to WAV format with 16 kHz sample rate
    converted_file_location = f"{UPLOAD_DIRECTORY}/converted.wav"
    try:
        convert_to_wav(file_location, converted_file_location)
    except Exception as e:
        return {"text": f"Conversion failed: {str(e)}"}

    # Process the audio file and return the result
    try:
        result = recognize_speech(converted_file_location)
    except Exception as e:
        return {"text": f"Recognition failed: {str(e)}"}
